...
```powershell 
! Usuario: Usuario administrador local (requiere AES key de krbtgt)

# Ejecutar desde una consola cmd limpia como administrador 
# Te genera una nueva consola con el ticket inyectado 
...
```powershell 
❯ winrs -r:dcorp-dc cmd       # Ingresar con una cmd al server 
	❯ set username            
```
